...
class DQN:
    ...
    def _build_net(self):
        ...
        x = Dense(128, activation='relu')(x)
        self.q_eval = Dense(self.n_actions, activation='softmax')(x)
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            #log.logger.debug('input_obs = \n %s' % (str(observation)))
            action = np.argmax(actions_value)
            #log.logger.debug('chosn action = \n%s\n action=%d' % (str(actions_value), action))
        else:
    ...
    def learn(self):
        self.n_step += 1
        if (self.n_step < 80) or (self.n_step % 80 != 0):
            return
        log.logger.debug('dukl-Training ...')
        ...
        self.model2.fit(batch_memory[:, :self.n_features], q_target, epochs=1)
