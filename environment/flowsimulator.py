    ...
    def queuing(self, i, j):
        ...
        while True:
            ...
            if flow.in_msg_on_road_time == self.env.now:
                ...
                log.logger.debug(
                    'continue messages ... msg_on_road-%d-%d, delay=%f' % (i, j, flow.in_msg_on_road_time - last_time))
                yield self.env.timeout(flow.in_msg_on_road_time - last_time)
                last_time = flow.in_msg_on_road_time
            if len(self.env_nfs[ACS.msg_msc[flow.msg_id][flow.index]][flow.nf_inst_id*ACS.n_node+j].message_queue.items) + 1 > ACS.n_max_load_in_nf[ACS.msg_msc[flow.msg_id][flow.index]] or self.env_nfs[ACS.msg_msc[flow.msg_id][flow.index]][flow.nf_inst_id*ACS.n_node+j].is_alive == False:
                ...
                log.logger.debug('flow-%d-%d-%d arrival nf-%d-%d-%d at time %f' % (flow.ue_id, flow.pro_id, flow.msg_id, self.env_nfs[ACS.msg_msc[flow.msg_id][flow.index]][flow.nf_inst_id*ACS.n_node+j].loc_id, ACS.msg_msc[flow.msg_id][flow.index], self.env_nfs[ACS.msg_msc[flow.msg_id][flow.index]][flow.nf_inst_id*ACS.n_node+j].inst_id, self.env.now))
